(function () {
    ...
    function clearFrame(placeholderText) {
        frameGeneration += 1;
        ...
        frameContext.clearRect(0, 0, image.width, image.height);
        image.style.display = 'none';
        placeholder.style.display = 'block';
        if (placeholderText) placeholder.textContent = placeholderText;
    }
    ...
    function renderFrame(blob) {
        ...
        loader.onload = function () {
            if (stopped || generation !== frameGeneration) {
        ...
        BYDAuth.fetch('/api/dev-view/frame', {
            ...
            if (!response.ok) {
                return response.json().then(function (data) {
                    ...
                    error.sessionInvalid = response.status === 403;
                    error.remoteLocked = data.pixelCopyResult === 'LOCKED';
                    throw error;
                });
            }
