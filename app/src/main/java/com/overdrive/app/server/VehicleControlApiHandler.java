 ...
 * POST /api/vehicle/climate — power=SDK_FIRST with cloud fallback,
 * set_temp/set_fan=SDK_ONLY
 * GET /api/vehicle/climate-schedule — BOOKINGAIR booking list (cloud reported)
 * POST /api/vehicle/climate-schedule — BOOKINGAIR create/update/delete,
 * CLOUD_ONLY
 ...
 * POST /api/vehicle/media — media volume (AudioManager) + screen brightness
 * (setting HAL)
 * POST /api/vehicle/battery-heat — CLOUD_ONLY
 * GET /api/vehicle/charging-schedule — cloud state with local last-known
 * fallback
 * POST /api/vehicle/charging-schedule — { startChargeTime, endChargeTime,
 * chargeWay, enabled } CLOUD_ONLY
 * POST /api/vehicle/start-charging — CLOUD_ONLY, terminally confirmed
 * GET /api/vehicle/charge-cap — { percent, enabled, supported } SDK_ONLY
 * (verified charge-stop backend)
 * POST /api/vehicle/charge-cap — { percent? 50..100, enabled? } SDK_ONLY
 * (verified charge-stop backend)
 * GET /api/vehicle/ac-charge-current-limit — { state, label, supported }
 * SDK_ONLY
 * POST /api/vehicle/ac-charge-current-limit — { state: 1..5 } SDK_ONLY,
 * readback verified
 */
public class VehicleControlApiHandler {
    ...
    public static boolean handle(String method, String path, String body, OutputStream out) throws Exception {
        ...
        // Nav / move shell out as UID 2000 (input keyevent / am start); screenshot
        // captures
        // in-process by layer stack (the a11y takeScreenshot route is API 30+, and
        // screencap's
        // -d takes a PhysicalDisplayId that cannot address the cluster's virtual
        // display).
        ...
        // POST /api/vehicle/cluster-resize — { l,t,r,b } in CLUSTER px. Resize the LIVE
        // cast
        // app ON the cluster to those bounds (freeform). No-op unless the feature flag
        // is on
        // and a cast is active (then the app stays fullscreen). Distinct from
        // cluster-mirror,
        ...
        // POST /api/vehicle/cluster-window — { package, l,t,r,b } as panel FRACTIONS
        // (0..1).
        // SAVES a per-app box without touching any live cast, so a cast started with no
        // Projection screen in the loop (ACC-on auto-start, key mapping, automation)
        // can still
    ...
    /**
     * List launchable apps for the cast picker (reuses the shared AppLauncher
     * enum), MINUS
     * OverDrive itself.
     *
     * <p>
     * OverDrive declares a LAUNCHER activity, so the shared enumeration includes
     * our own
     * package — but casting ourselves onto the cluster is never what the user wants
     * and is
     * actively hazardous: {@code com.overdrive.app} also owns the head-unit UI task
     * AND the
     * nav-map cluster task ({@code .navmap.RoadSenseClusterMapActivity}), so a
     * self-cast makes
     * "the cast app's task" ambiguous for the freeform resize path. That path is
     * display-scoped
     * and so resolves correctly regardless, but removing the entry eliminates the
     * ambiguity at
     * the source rather than relying on the downstream guard. Filtered HERE (not in
     * {@code listLaunchableApps}) so the automation / key-mapping pickers, where
     * launching our own
    ...
    private static void handleClusterApps(OutputStream out) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < all.length(); i++) {
                ...
                if (app == null)
                    continue;
                if (self.equals(app.optString("package", "")))
                    continue;
                apps.put(app);
            }
            response.put("success", true);
    ...
    /**
     * Cast a package onto the driver cluster (acquires projection, resolves the
     * live
     * fission id, launches fullscreen, holds it open). Reuses the proven
     * ClusterCast.
     */
    ...
    /**
     * Stop casting + tear down the mirror; the controller restores the gauges when
     * no
    ...
    /**
     * Resize the LIVE cast app ON the cluster to the given CLUSTER-px bounds
     * (freeform).
     * Returns {@code applied=false} (not an error) when the feature is off or no
     * cast is
     * active — the caller treats that as "app stays fullscreen".
     *
     * <p>
     * {@code commit} (default true) selects the apply strategy: true = gesture
     * release /
     * preset / restore, running the full escalation ladder and VERIFYING that AMS
     * honoured the
     * rect; false = a live drag frame, issuing a cheap bounds-only update so the
     * window tracks
    ...
    /**
     * Persist a per-app cluster box (panel fractions 0..1) for later UI-less casts.
     * Purely a
     * config write — it never resizes a live cast (that is {@code cluster-resize}),
     * so the
     * Projection screen can call it while nothing is being cast.
     * {@code saved=false} means the
     ...
     * { "action": "start"|"rect"|"stop", "x":px,"y":px,"w":px,"h":px } (head-unit
     * px).
     */
    private static void handleClusterMirror(OutputStream out, String body) throws Exception {
        ...
        try {
            ...
            com.overdrive.app.surveillance.ClusterMirrorController ctl = com.overdrive.app.surveillance.ClusterMirrorController
                    .getInstance();
            // Optional scaling mode (fit=0 default / fill=1 / zoom=2), sent on start/rect
            // and
    ...
    /**
     * Cast state + real cluster panel size for the Projection screen. The mirror
     * itself is
     * driven over the ClusterViewMirrorService Binder channel (a Surface can't ride
     * HTTP),
     * so this endpoint only reports the cast flag + the panel aspect the UI locks
     * its box to.
     */
    private static void handleClusterMirrorStatus(OutputStream out) throws Exception {
        ...
        try {
            ...
            // WHICH package is on the cluster. The UI needs this because the resize box
            // acts on the
            // CAST app, while the spinner reflects the user's next PICK — the two diverge
            // as soon as
            // the user browses the list during a live cast. Without it the UI would key its
            // geometry
            // (and its restore-on-select) to the spinner and reshape the cast app to
            // another app's
            ...
            int pw = 0, ph = 0;
            try {
                com.overdrive.app.surveillance.ClusterViewMirrorService vm = com.overdrive.app.surveillance.ClusterViewMirrorService
                        .getInstance();
            ...
            if (pw <= 1 || ph <= 1) {
                try {
                    ...
                    if (ctx != null) {
                        android.graphics.Point p = com.overdrive.app.surveillance.BsNativeLayer.clusterDisplaySize(ctx);
                        pw = p.x;
                        ph = p.y;
                    }
                } catch (Throwable ignored) {
    ...
    /**
     * Relay a tap/swipe (mirror-SURFACE px, sx/sy) into the projected app; the
     * relay inverts
    ...
    private static void handleClusterTouch(OutputStream out, String body) throws Exception {
        ...
        try {
            ...
            response.put("success", ok);
            if (!ok)
                response.put("error", "no safe cluster display target");
        } catch (Exception e) {
    ...
    private static void handleGetState(OutputStream out) throws Exception {
        ...
        if (data == null) {
            ...
            try {
                com.overdrive.app.byd.cloud.BydCloudDataProvider provider = com.overdrive.app.byd.cloud.BydCloudDataProvider
                        .getInstance();
                ...
                if (cs != null) {
                    ...
                    if (cs.hasChargingState())
                        b.chargingState(cs.getChargingStateAsSdk());
                }
                data = b.build();
            } catch (Throwable ignored) {
            }
            ...
            if (ctx != null) {
                ...
                if (otaDevice != null) {
                    ...
                    if (v instanceof Number) {
                        ...
                        if (sdkState == 2)
                            otaLf = 1; // LOCKED → API=1
                        else if (sdkState == 1)
                            otaLf = 2; // UNLOCKED → API=2
                    }
                }
            }
        } catch (Throwable t) {
        ...
        if (!Double.isNaN(data.socPercent))
            battery.put("soc", data.socPercent);
        ...
        // Ambient main switch as a tri-state: true / false / omitted-when-unreadable,
        // so a UI
        ...
        // SDK value: 1=on, 2=off, 3=delay. Treat on(1) and delay(3) as enabled;
        // anything else —
        // off(2) or the unpopulated default 0 on vehicles that don't report CPD — reads
        // as off, so
        ...
        // Steering-wheel heater. Local read first (2=on/1=off), then the cloud
        // snapshot's
        // own wire domain. The key is omitted entirely when neither source answered, so
        // the
        ...
        if (wheelHeat == 1 || wheelHeat == 2) {
            ...
            try {
                com.overdrive.app.byd.cloud.VehicleCloudSnapshot cs = com.overdrive.app.byd.cloud.BydCloudDataProvider
        ...
        // SDK readback — so the key is omitted unless a fresh snapshot reported it.
        // Without
        // this the UI had no state at all: after a reload every tap re-sent "on",
        // leaving
        ...
        // remote climate session), gate acOn to false and fanLevel to 0 so the
        // dashboard
        ...
        if (com.overdrive.app.byd.DiLink5Platform.isActive()) {
            ...
            if (carSvcTemps[0] >= 0)
                climate.put("driverTempSetC", carSvcTemps[0]);
            if (carSvcTemps[1] >= 0)
                climate.put("passengerTempSetC", carSvcTemps[1]);
     ...
     * Open: cloud unlock → SDK tailgate (router enforces the safety: motor only
     * fires on unlock SUCCESS).
    ...
    private static void handleTrunk(OutputStream out, String body) throws Exception {
        ...
        if ("close".equals(action))
            ...
            cmd = new VehicleCommandRouter.TrunkOpenCommand();
        else {
            response.put("success", false);
     ...
     * exposed separately so a successful remote vent is never reported as full
     * open.
     */
    private static void handleWindow(OutputStream out, String body) throws Exception {
        ...
        try {
            ...
            // 1=open, 2=close, 3=stop. Nothing downstream bounds this: WindowMoveCommand
            // does not
            // validate and setAllWindowsCommand has no range check, so command=0 wrote the
            // HAL's own
     ...
     * power_on / power_off → SDK_FIRST (local HVAC, then OPENAIR / CLOSEAIR
     * fallback).
    ...
    private static void handleClimate(OutputStream out, String body) throws Exception {
        ...
        try {
            ...
            switch (action) {
                case "power_on": {
                    ...
                    Integer remoteDuration = req.has("remoteDurationMinutes")
                            ? jsonInteger(req, "remoteDurationMinutes")
                            : Integer.valueOf(20);
                ...
                // command (from the UI, HA, or a second automation step). Zero cancels;
                // positive
                // values arm a timer. See AcAutoOffTimer for the single-timer / last-write-wins
                // rules.
                case "auto_off_timer": {
                ...
                case "step_temp": {
                    ...
                    if (areaValue == null
                            || (areaValue.intValue() != com.overdrive.app.byd.BydDataCollector.AC_TEMP_AREA_DRIVER
                                    && areaValue
                                            .intValue() != com.overdrive.app.byd.BydDataCollector.AC_TEMP_AREA_PASSENGER)) {
                    ...
                    VehicleCommandRouter.ClimateStepTempCommand step = new VehicleCommandRouter.ClimateStepTempCommand(
                            zone, area, delta);
                ...
                case "auto_on":
                    cmd = new VehicleCommandRouter.AcAutoModeCommand(true);
                    break;
                case "auto_off":
                    cmd = new VehicleCommandRouter.AcAutoModeCommand(false);
                    break;
                case "sync_on":
                    cmd = new VehicleCommandRouter.AcTemperatureSyncCommand(true);
                    break;
                case "sync_off":
                    cmd = new VehicleCommandRouter.AcTemperatureSyncCommand(false);
                    break;
                case "fan_only_on":
                    cmd = new VehicleCommandRouter.FanOnlyModeCommand(true);
                    break;
                case "fan_only_off":
                    cmd = new VehicleCommandRouter.FanOnlyModeCommand(false);
                    break;
                case "steering_heat_on":
                    cmd = new VehicleCommandRouter.SteeringWheelHeatCommand(true);
                    break;
                case "steering_heat_off":
                    cmd = new VehicleCommandRouter.SteeringWheelHeatCommand(false);
                    break;
                case "recirculate_on":
                    cmd = new VehicleCommandRouter.AcRecirculationCommand(true);
                    break; // recirculation
                case "recirculate_off":
                    cmd = new VehicleCommandRouter.AcRecirculationCommand(false);
                    break; // fresh air
                case "defrost_front_on":
                    cmd = new VehicleCommandRouter.FrontDefrostCommand(true);
                    break;
                case "defrost_front_off":
                    cmd = new VehicleCommandRouter.FrontDefrostCommand(false);
                    break;
                case "defrost_rear_on":
                    cmd = new VehicleCommandRouter.RearDefrostCommand(true);
                    break;
                case "defrost_rear_off":
                    cmd = new VehicleCommandRouter.RearDefrostCommand(false);
                    break;
                default:
            ...
            // Auto-off timer bookkeeping, applied only when the command actually took
            // effect
            // so a refused/failed write never leaves a timer that would switch off an AC
            // this
            ...
            if (r.outcome == VehicleCommandRouter.Outcome.SUCCESS) {
                ...
                // NOTE: power_off does NOT need to cancel the window here —
                // VehicleCommandRouter
                // retires it centrally on any successful ClimateOffCommand, so every surface
                // (this
     ...
     *
     * <p>
     * Body: {@code { action: "create"|"update"|"delete", bookingId?,
     * bookingTime?, temp?, durationMinutes? }}. Times are epoch seconds.
     * Create/update require a
     * future booking time, 15..31 C whole-degree temperature, and one of the OEM's
     * duration
    ...
    private static void handleClimateSchedule(OutputStream out, String body) throws Exception {
        ...
        try {
            ...
            if (result.outcome == VehicleCommandRouter.Outcome.SUCCESS) {
                ...
                if (bookingId != null)
                    resp.put("bookingId", String.valueOf(bookingId.longValue()));
                if (bookingTime != null)
                    resp.put("bookingTime", bookingTime.longValue());
                if (temp != null)
                    resp.put("temp", temp.doubleValue());
                if (duration != null)
                    resp.put("durationMinutes", duration.intValue());
            } else {
                ...
                if (bookingTime != null)
                    resp.put("requestedBookingTime", bookingTime.longValue());
            }
            HttpResponse.sendJson(out, resp.toString());
        } catch (Exception e) {
    ...
    /**
     * Cloud-reported BOOKINGAIR schedules. An empty list is explicitly not a
     * deletion proof.
     */
    private static void handleGetClimateSchedule(OutputStream out) throws Exception {
        JSONObject response = new JSONObject();
        try {
            com.overdrive.app.byd.cloud.BydCloudConfig cfg = com.overdrive.app.byd.cloud.BydCloudConfig
                    .fromUnifiedConfig();
            ...
            com.overdrive.app.byd.cloud.BydCloudClient client = com.overdrive.app.byd.cloud.BydCloudDataProvider
            ...
            // outright. Report supported=true first, then attach the bookings if we got
            // them.
            response.put("success", true);
    ...
    /**
     * BYD booking IDs are 64-bit and commonly exceed JavaScript's exact-integer
     * range. Return
     * them as decimal text so a list response can be fed straight back into
     ...
     *
     * <p>
     * {@code level} is 0=off / 1=low / 2=high — the HAL has three states. The doc
     * previously
     * advertised 0-3; a 3 was accepted, silently collapsed to 2, and echoed back as
     * 3, so a caller
     * was told a level the seat never reached. Out-of-range is now refused.
     *
     * <p>
     * "position" recalls a stored driver-seat memory slot (1-2); "save" stores
    ...
    private static void handleSeat(OutputStream out, String body) throws Exception {
        ...
        try {
            ...
            if (levelValue != null)
                resp.put("level", levelValue.intValue());
            HttpResponse.sendJson(out, resp.toString());
        } catch (Exception e) {
    ...
    private static void handleLights(OutputStream out, String body) throws Exception {
        ...
        try {
            ...
            if ("dayTimeLight".equals(target)) {
                ...
                // Optional zone (front/rear/both); default both keeps prior whole-cabin
                // behaviour.
                ...
                // clamped to 11 and answered success:true, so the caller was told a level it
                // never
    ...
    private static void handleAdas(OutputStream out, String body) throws Exception {
        ...
        try {
            ...
            resp.put("target", target);
            if (enabled != null)
                resp.put("enable", enabled.booleanValue());
            if (mode != null)
                resp.put("mode", mode.intValue());
            HttpResponse.sendJson(out, resp.toString());
        } catch (Exception e) {
     ...
     * the raw SDK int plus a best-effort parsed on/off (1=on, 0=off;
     * -1=unavailable).
     */
     ...
     * { "target": "cluster_brightness", "value": 0-100 } → driver-cluster
     * brightness
     ...
     * (media/navigation/voice/phone/system/alarm/ring); default "media"
     * (STREAM_MUSIC)
        ...
        try {
            ...
            if (isVolume || isVolumeStep) {
                ...
                if (volumeChannel == null) {
                    response.put("success", false);
                    response.put("error",
                            "volume channel must be media, navigation, voice, phone, call, system, alarm, or ring");
                    HttpResponse.sendJson(out, response.toString());
                    return;
                }
            }
            ...
            if (isMediaKey) {
                ...
                // global main switch (three-tier chain); a single zone has no dedicated switch,
                // so
    ...
    /**
     * Show an on-screen toast or dialog via the app-process
     * {@code MessageOverlayService}
     * (the daemon has no UI surface, same reason speak/play-video are bridged out).
     * Body:
    ...
    private static void handleMessage(OutputStream out, String body) throws Exception {
        ...
        try {
            ...
            String validation = com.overdrive.app.communication.RemoteCommunicationPolicy
            ...
            String requestedKind = com.overdrive.app.communication.RemoteCommunicationPolicy
                    .normalizeKind(req.optString("kind", "toast"));
            String kind = com.overdrive.app.communication.RemoteCommunicationPolicy
                    .effectiveKind(
                            requestedKind,
                            com.overdrive.app.communication.VehicleCommunicationSafety.isParked());
            String severity = com.overdrive.app.communication.RemoteCommunicationPolicy
    ...
    /**
     * User-visible screenshot folder; the daemon runs as shell and can write shared
     * storage.
     */
     ...
     * Nav is a fire-and-forget shell exec (`input keyevent`, never blocking the
     * request thread
     * on a hung child); screenshot captures in-process via {@link
     * com.overdrive.app.surveillance.DisplayScreenshot}; move uses `am
     * start-activity`.
     */
        ...
        try {
            ...
            switch (target) {
                ...
                case "screenshot": {
                    ...
                    android.content.Context ctx = com.overdrive.app.daemon.CameraDaemon.getAppContext();
                    com.overdrive.app.surveillance.DisplayScreenshot.Result shotResult = (display == 1)
                ...
                case "move_display": {
                    ...
                    response.put("target", target);
                    if (!moved)
                        response.put("error", "could not move app (bad package or unresolved component)");
                    HttpResponse.sendJson(out, response.toString());
                    return;
                }
    ...
    /**
     * Run a shell command as the daemon (UID 2000), bounded so a hang can't park
     * us.
     */
    ...
    // sounds picked by the "Play Audio" action live. A "name" payload resolves
    // here.
    private static final String AUDIO_LIBRARY_DIR = "/data/local/tmp/.overdrive/audio";

    /**
     * Play an uploaded sound (by library {@code name}) or an explicit {@code path}
     * on
     ...
     * path (advanced). Channel defaults to "media". The controller validates the
     * file
    ...
    private static void handlePlayAudio(OutputStream out, String body) throws Exception {
        ...
        try {
            ...
            // VideoPlaybackActivity (a TextureView-backed player). Its MediaPlayer must
            // keep
            // the default Media attributes or this DiLink build can play audio with no
            // video
            ...
            if (!ok)
                response.put("error", "could not play (missing/unreadable file, or playback unavailable)");
            HttpResponse.sendJson(out, response.toString());
        } catch (Exception e) {
    ...
    /**
     * Map a channel name to its Android {@code AudioManager.STREAM_*} type.
     * Channel→
     ...
     * STREAM_NOTIFICATION-independent choice below is avoided — see mapping).
     * Unknown
    ...
    private static int streamForChannel(String channel) {
        ...
        switch (channel.trim().toLowerCase()) {
            ...
            case "navigation":
                return 14; // STREAM_NAVI — OEM nav-guidance stream (the OEM app setBroadcastVolume uses
                           // 14)
            case "voice":
                return 16; // OEM voice stream (the OEM app setVoiceVolume uses 16)
    ...
    /**
     * Set the given audio channel's volume to an ABSOLUTE step index via
     * AudioManager
     * on the daemon's app context — the same 0..max scale as the car's own volume
     * button (media max is 40 on this head unit). The index is clamped to the
     * stream's
     * real max so a too-high value pins to max rather than failing. Returns false
     * when
    ...
    private static boolean setChannelVolumeIndex(String channel, int index) {
        try {
            ...
            android.media.AudioManager am = (android.media.AudioManager) ctx
    ...
    /**
     * Write the OEM "volume_music" AudioManager parameter for the media stream —
     * the
     * lever the head-unit firmware itself uses for media volume (mirrors the OEM
     * firmware's setMediaVolumeViaParameters, which tries these three forms). The
     * "8"
     * is the OEM media stream id. Best-effort: setParameters is a fire-and-forget
     * OEM
     * hook that may be a no-op on trims that don't recognise it, so failures are
     * swallowed — the setStreamVolume write already ran as the standard-Android
     * path.
     */
    ...
    /**
     * Step the given channel's volume by one index (dir &gt; 0 up, &lt; 0 down) via
     * a
     ...
     * adjustStreamVolume so the same authoritative path as the absolute action is
     * used.
     */
     ...
     * {@code input keyevent <code>} run as the UID-2000 daemon shell — the same
     * privileged
     * injection the keymap replay
     * ({@code KeymapApiHandler}/{@code KeyMapDispatcher}) and
     * {@code handleSystem} (HOME/BACK/APP_SWITCH) already use successfully.
     *
     * <p>
     * The previous {@link android.media.AudioManager#dispatchMediaKeyEvent} path
     * failed
     * for the real sources (Android Auto / Bluetooth / DAB radio): dispatched from
     * the
     * daemon's synthetic, non-foreground app context it is not delivered to the
     * media
     * session that owns audio focus. A system-level {@code input keyevent}
     * injection is
     * routed by the OS to the focused/audio-focus owner regardless of our caller
     * identity.
     *
     * <p>
     * Uses explicit PLAY(126)/PAUSE(127) rather than the PLAY_PAUSE toggle for the
     * play/pause action's underlying media codes only where a fixed intent is
     * known; the
     * "play_pause" action keeps the toggle keycode (85) since it is an explicit
     * toggle
     * request. Codes match the OEM vehicle-control app (mediaNext=87,
     * mediaPrevious=88). Returns false on
     * unknown key.
     */
    private static boolean dispatchMediaKey(String key) {
        ...
        switch (key == null ? "" : key.trim().toLowerCase()) {
            ...
            case "pause":
                ...
                break; // 127
            case "next":
                code = android.view.KeyEvent.KEYCODE_MEDIA_NEXT;
                break; // 87
            case "previous":
            case "prev":
        ...
        // input keyevent injects a complete DOWN+UP press at the input layer; no
        // context /
        // AudioManager needed. runDetachedShell bounds the wait so a hung `input` can't
        // park us.
    ...
    private static void handleGetChargingSchedule(OutputStream out) throws Exception {
        ...
        try {
            ...
            Boolean enabled = cached.has("enabled") && !cached.isNull("enabled")
                    ? Boolean.valueOf(cached.optBoolean("enabled"))
                    : null;
            ...
            if (enabled == null)
                resp.put("enabled", JSONObject.NULL);
            else
                resp.put("enabled", enabled.booleanValue());
            ...
            if (journey != null)
                resp.put("smartJourneyDto", journey);
     ...
     *
     * <p>
     * Body: {@code { percent?: 50..100, enabled?: bool }}.
        ...
        try {
            ...
            if (hasPercent && !hasEnabled) {
                ...
                if (r.outcome != VehicleCommandRouter.Outcome.SUCCESS) {
                    ...
                    if (supported != null)
                        resp.put("supported", supported.booleanValue());
    ...
    private static void handleGetChargeCap(OutputStream out) throws Exception {
        ...
        try {
            ...
            resp.put("percent", verified && percent >= 50 && percent <= 100
                    ? percent
                    : JSONObject.NULL);
            if (verified && enabled == 0)
                resp.put("enabled", false);
            else if (verified && enabled == 1)
            ...
            if (supported == null)
                resp.put("supported", JSONObject.NULL);
            else
                resp.put("supported", supported.booleanValue());
    ...
    /**
     * Attach only verified SDK constraints so the UI never advertises an unproven
     * range.
     */
    private static void appendChargeCapConstraints(JSONObject response, BydDataCollector collector)
        ...
        if (kind != null && !"unknown".equals(kind))
            response.put("controlKind", kind);
    }
    ...
    private static void handleGetAcChargeCurrentLimit(OutputStream out) throws Exception {
        ...
        try {
            BydDataCollector collector = BydDataCollector.getInstance();
            BydDataCollector.AcChargingCurrentLimitStatus status = collector.getAcChargingCurrentLimitStatus();
            int state = status.state;
            response.put("success", true);
            response.put("supported", status.supported != null
                    ? status.supported.booleanValue()
                    : JSONObject.NULL);
    ...
    private static void handleAcChargeCurrentLimit(
        ...
        try {
            ...
            // The final HAL state is the automation/manual command contract. This also
            // handles a
            // delayed app-process retry: it may apply after the daemon-side route timed
            // out, or a
    ...
    private static Boolean jsonBoolean(JSONObject request, String key) {
        if (request == null || !request.has(key) || request.isNull(key))
            return null;
    ...
    /**
     * Strict JSON integer: numeric JSON only, with no coercion or fractional
     * values.
     */
     ...
     *
     * <p>
     * Automation form fields are strings, and a 64-bit BOOKINGAIR id cannot safely
     * pass
     * through a browser JSON number. Accepting a digits-only string preserves every
     * bit without
     * permitting coercion, fractions, exponent notation, whitespace, or a
     * JavaScript double.
     */
    private static Long jsonLong(JSONObject request, String key) {
        ...
        if (!(value instanceof Number))
            return null;
        // JSONObject parses integer JSON literals as integral Number types. Preserve
        // those values
        // directly: booking ids are 64-bit and routing them through double would round
        // their low
        ...
        if (Double.isNaN(raw) || Double.isInfinite(raw) || raw != Math.rint(raw)
                ...
                // A JSON floating point value above 2^53 cannot represent every integer
                // exactly.
    ...
    /**
     * Optional ambient zones default to both only when absent. A malformed value
     * must never map
     * to the collector's catch-all area, which would unexpectedly control the
     * entire cabin.
     */
    private static String optionalAmbientZone(JSONObject request) {
        if (request == null || !request.has("zone"))
            return "both";
    ...
    /**
     * Volume controls default to media only when channel is absent. Unknown
     * channels must not
     * reach streamForChannel's legacy media default, which would control the wrong
     * stream.
     */
    ...
    /**
     * The router may expose this optional state while server and router artifacts
     * are updated
    ...
    private static boolean isValidChargingTime(String value, boolean allowFull) {
        if (allowFull && "full".equals(value))
            return true;
    ...
    private static boolean isValidChargeWay(String value) {
        ...
        for (String day : days) {
            if (day.length() != 1 || day.charAt(0) < '0' || day.charAt(0) > '6')
                return false;
    ...
    /**
     * Start from the collector's full state and change exactly the requested zone.
     * Callers use
     * this only after {@link #hasFreshCompleteSeatState(BydVehicleData)} succeeds;
     * request-body
