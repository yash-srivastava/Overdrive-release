 ...
 * ({@code /sendlog camera}) and to pass as a query param ({@code ?daemon=camera}).
 *
 * Paths are resolved via {@link ScratchPaths} at call time so Sealion keeps
 * legacy tmp when writable and Shark falls back after the probe.
 */
public final class DaemonLogPaths {
    ...
    // Values are scratch-relative names, NOT absolute paths.
    ...
    public static String pathFor(String key) {
        ...
        return rel == null ? null : ScratchPaths.path(rel);
    }
